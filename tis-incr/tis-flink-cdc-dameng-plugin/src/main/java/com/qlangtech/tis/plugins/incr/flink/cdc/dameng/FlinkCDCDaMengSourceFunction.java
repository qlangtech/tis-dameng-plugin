 ...
 **/
public class FlinkCDCDaMengSourceFunction implements IMQListener<List<ReaderSource>> {

    private final FlinkCDCDaMengSourceFactory sourceFactory;
    ...
    public AsyncMsg<List<ReaderSource>> start(IConsumerRateLimiter streamFactory, boolean flinkCDCPipelineEnable,
                                              DataXName channalName, IDataxReader dataSource, List<ISelectedTab> tabs
            , IDataxProcessor dataXProcessor) throws MQConsumeException {
        try {
            ...
            for (ISelectedTab tab : tabs) {
                FlinkColMapper colsMapper = AbstractRowDataMapper.getAllTabColsMetaMapper(tab.getCols(),
                        flinkColCreator);
            ...
            final TISDeserializationSchema deserializationSchema =
                    new TISDeserializationSchema(new DaMengSourceDTOColValProcess(tabColsMapper),
                            tablesInDB.getPhysicsTabName2LogicNameConvertor(), contextParamValsGetterMapper);
            ...
            SourceChannel sourceChannel = new SourceChannel(flinkCDCPipelineEnable,
                ...
                return dbs.getDbStream().map((databaseName) -> {
                    ...
                    debeziumProperties.setProperty(DamengConnectorConfig.LOG_MINING_STRATEGY.name(),
                            DamengConnectorConfig.LogMiningStrategy.CATALOG_IN_REDO.getValue());//
                    // "online_catalog");
                    ...
                    for (Triple<String, Field, Function<FlinkCDCDaMengSourceFactory, Object>> t :
                            FlinkCDCDaMengSourceFactory.debeziumProps) {
                        debeziumProperties.setProperty(t.getMiddle().name(),
                                String.valueOf(t.getRight().apply(sourceFactory)));
                    }
                    ...
                    debeziumProperties.setProperty( //
                            ...
                            , DaMengConnectionFactory.class.getCanonicalName());
                    for (HostDB hostDB : dbs.dbs) {
                        debeziumProperties.setProperty(DATABASE_CONFIG_PREFIX + DataxUtils.DATASOURCE_FACTORY_IDENTITY //
                                , Objects.requireNonNull(dsFactory.identityValue(),
                                        "datasource identity can not be " + "null"));
                        debeziumProperties.setProperty(DATABASE_CONFIG_PREFIX + DataxUtils.DATASOURCE_JDBC_URL,
                                hostDB.getJdbcUrl());
                        break;
                    }
                    ...
                    builder.hostname(dbHost) //
                            ...
                            .port(dsFactory.port) //
                            .serverTimeZone(dsFactory.getTimeZone().orElseThrow().getId())//
                    ...
                    return ReaderSource.createDTOSource(streamFactory, channalName, dbHost + ":" + dsFactory.port +
                            "_" + databaseName, flinkCDCPipelineEnable, sourceFunction);
                }).collect(Collectors.toList());

            }));

            sourceChannel.setFocusTabs(tabs //
                    , dataXProcessor.getTabAlias(null), (tabName) -> DTOStream.createDispatched(tabName,
                            sourceFactory.independentBinLogMonitor));
